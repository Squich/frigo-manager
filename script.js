...
const formatDateShort = (str) =>
  str.replace(/(\d{4})-(\d{2})-(\d{2})/g, "$3/$2");

/* Generate startList */

const generateDate = (num) =>
  formatDateLong(new Date(new Date().getTime() + num));

const startList = [
  { name: "Saumon", date: generateDate(-d * 9), recipe: "", opened: "true" },
  {
    name: "Fromage rapé",
    date: generateDate(-d * 3),
    recipe: "Omelette",
    opened: "false"
  },
  {
  ...
  {
    name: "Lardons",
    date: generateDate(d * 2),
    recipe: "Salade composée",
    opened: "false"
  },
    ...
    .forEach((item, i) => {
      ...
      listItem.innerHTML = `
			...
			<span>
				${
          item.recipe
            ? '<span class="recipe-container"><i class="icon-recipe fa fa-tags"></i></span>'
            : ""
        }
      ...
      tooltipDLC.textContent =
        diff(item.date) + " jr" + (item.opened === "false" ? "" : " (entamé)");
      ...
      smiley.className =
        "smiley fa " +
        chooseSmiley(
          item.opened === "false" || !item.opened ? diff(item.date) : Infinity
        ) +
        " " +
        colorizeSmiley(diff(item.date));
...
const getListItems = () =>
  (listItems = JSON.parse(localStorage.getItem("listItems")) || startList);

/* Display a message if the list is empty */

const checkEmpty = () =>
  listItems.length
    ? (empty.classList.remove("active"), btnDeleteAll.classList.add("active"))
    : (empty.classList.add("active"), btnDeleteAll.classList.remove("active"));
...
const today = () =>
  (date.textContent = formatDateShort(formatDateLong(new Date())));
...
const chooseSmiley = (num) =>
  num < 0
    ...
    ? "fa-exclamation"
    : "fa-smile-o";
const colorizeSmiley = (num) =>
  num < 0 ? "black" : num < 2 ? "red" : num < 4 ? "orange" : "green";

/* Check the radio buttons */

const checkIfOpened = () =>
  newItemOpened === "true"
    ? (formOpenedYes.checked = true)
    : (formOpenedNo.checked = true);
...
list.addEventListener("click", (e) => {
  ...
  if (
    e.target.classList.contains("smiley") ||
    e.target.classList.contains("icon-recipe")
  ) {
...
formName.addEventListener(
  "change",
  () => (newItemName = formName.value.trim())
);
formDate.addEventListener("change", () => (newItemDate = formDate.value));
formRecipe.addEventListener(
  "keyup",
  () => (newItemRecipe = formRecipe.value.trim())
);
formRecipe.addEventListener(
  "change",
  () => (newItemRecipe = formRecipe.value.trim())
);
formOpened.forEach((btn) =>
  btn.addEventListener("change", () => (newItemOpened = btn.value))
);
